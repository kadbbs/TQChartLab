...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="使用天勤专业版历史下载能力，将 tick 数据归档到 DuckDB。")
    parser.add_argument("--db-path", type=Path, default=PROJECT_ROOT / "data" / "duckdb" / "ticks.duckdb")
    parser.add_argument(
        "--temp-dir",
        type=Path,
        default=PROJECT_ROOT / "data" / "duckdb" / "tmp_ticks",
        help="历史下载的临时 CSV 目录。",
    )
    ...
    parser.add_argument("--chunk-days", type=int, default=5, help="每次下载的日期块大小。")
    ...
    parser.add_argument(
        "--ignore-latest-ts",
        action="store_true",
        help="忽略库内最新时间，从 start-date 强制回填；重复 tick 会被唯一索引自动去重。",
    )
...
def select_contracts(
    ...
    for contract in catalog:
        contract_exchange_id = str(contract.get("exchange_id") or "")
        contract_product_id = str(contract.get("product_id") or "")
        symbol = str(contract["symbol"])
        if contract_exchange_id != exchange_id or contract_product_id != product_id:
            continue
...
def main() -> int:
    ...
    try:
        ...
        metadata_rows = [make_contract_metadata(args.provider, item) for item in contracts]
        store.upsert_contracts(metadata_rows)

        log(
            f"start duckdb tick archive contracts={len(contracts)} "
        ...
        for index, contract_dict in enumerate(contracts, start=1):
            ...
            latest_ts = None if args.ignore_latest_ts else store.get_latest_timestamp(contract.provider, contract.symbol)
            ...
            for window_start, window_end in daterange_chunks(symbol_start, end_date, max(args.chunk_days, 1)):
                ...
                downloader.download_tick_csv(contract.symbol, window_start, window_end, csv_path)
                inserted_rows = store.import_tick_csv(
                    csv_path=csv_path,
                    contract=contract,
